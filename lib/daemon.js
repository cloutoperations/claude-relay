...
// --- Load instance-level system prompt (e.g. copilot preamble) ---
var instancePrompt = null;
if (config.systemPromptFile) {
  try {
    var promptPath = require("path").resolve(require("./config").CONFIG_DIR, config.systemPromptFile);
    instancePrompt = require("fs").readFileSync(promptPath, "utf8");
...
process.on("uncaughtException", function (err) {
  // EPIPE/ECONNRESET are harmless — socket closed by client. Don't crash.
  if (err && (err.code === "EPIPE" || err.code === "ECONNRESET" || err.code === "ERR_STREAM_DESTROYED")) {
    console.warn("[daemon] Ignored socket error:", err.code);
    return;
  }
