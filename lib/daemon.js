...
if (config.claudeConfigDir) {
  // Backward compat: filter to just the configured account
  allAccounts = allAccounts.filter(function (a) { return a.configDir === config.claudeConfigDir; });
  if (allAccounts.length === 0) {
    // Configured dir not found in detected accounts; still honour it
    allAccounts = [{ configDir: config.claudeConfigDir, name: "configured", email: "unknown" }];
  }
}
