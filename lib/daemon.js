...
// --- Multi-account: propagate CLAUDE_CONFIG_DIR to child processes (SDK) ---
var accountLabel = null;
if (config.claudeConfigDir) {
  process.env.CLAUDE_CONFIG_DIR = config.claudeConfigDir;
  console.log("[daemon] Claude account:", config.claudeConfigDir);
}
// Read account email for UI badge
(function () {
  var configDir = config.claudeConfigDir || path.join(require("os").homedir(), ".claude");
  try {
    var claudeJson = JSON.parse(fs.readFileSync(path.join(configDir, ".claude.json"), "utf8"));
    if (claudeJson.oauthAccount && claudeJson.oauthAccount.emailAddress) {
      accountLabel = claudeJson.oauthAccount.emailAddress;
    }
  } catch (e) {}
})();
