...
function createProjectContext(opts) {
  ...
  var FILE_INDEX_TTL = 10000; // 10 seconds
    ...
    function walk(dir, relPrefix) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        if (item.isDirectory()) {
          ...
          entries.push({ name: item.name, nameLower: item.name.toLowerCase(), path: rel });
        }
      }
    }
    walk(cwd, "");
  ...
  function handleConnection(ws) {
    ...
    // Send cached state
    sendTo(ws, { type: "info", cwd: cwd, slug: slug, project: title || project, version: currentVersion, debug: !!debug, dangerouslySkipPermissions: dangerouslySkipPermissions, lanHost: lanHost, accountLabel: accountLabel, projectCount: getProjectCount(), projects: getProjectList() });
    if (latestVersion) {
  ...
  function setTitle(newTitle) {
    title = newTitle || null;
    send({ type: "info", cwd: cwd, slug: slug, project: title || project, version: currentVersion, debug: !!debug, lanHost: lanHost, accountLabel: accountLabel, projectCount: getProjectCount(), projects: getProjectList() });
  }
