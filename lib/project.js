  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "stop") {
      ...
      if (session && session.isProcessing) {
        // Try graceful interrupt first, then abort
        ...
        // Auto-escalate: if still processing after 5s, force-kill the subprocess
        var sessionRef = session;
        setTimeout(function () {
          if (sessionRef.isProcessing && sessionRef.queryInstance) {
            console.log("[project] Stop did not resolve after 5s, force-killing subprocess");
            ...
            sm.sendAndRecord(sessionRef, { type: "info", text: "Interrupted (force-stopped after timeout)" });
            sm.sendAndRecord(sessionRef, { type: "done", code: 1 });
            sm.broadcastSessionList();
          }
        }, 5000);
      }
