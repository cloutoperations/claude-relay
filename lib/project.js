...
function createProjectContext(opts) {
  ...
  function handleConnection(ws) {
    ...
    // Warmup deferred to first actual user query (startQuery).
    // sdk.warmup() spawns Claude subprocesses that block the event loop,
    // so never call it during handleConnection — it freezes the daemon.
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "set_session_status") {
      ...
      if (statusId && sm.sessions.has(statusId) && ["open", "done", "waiting"].indexOf(newStatus) !== -1) {
        var statusSession = sm.sessions.get(statusId);
        statusSession.status = newStatus;
        sm.saveSessionFile(statusSession);
        sm.broadcastSessionList();
      }
      return;
    }
    ...
    if (msg.type === "heal_sessions") {
      // Force-save all in-memory sessions to disk (including expired ones)
      var healed = 0;
      var errors = 0;
      sm.sessions.forEach(function (session) {
        if (!session.cliSessionId) return;
        try {
          // Temporarily allow saving even if _tempId is set
          var wasTempId = session._tempId;
          var wasStatus = session.status;
          if (wasTempId && wasStatus === "expired") {
            // Already handled by the updated saveSessionFile
          }
      ...
      sendTo(ws, { type: "heal_sessions_result", healed: healed, errors: errors, total: sm.sessions.size });
    ...
    if (msg.type === "fs_read") {
      ...
      var fsFile = safePath(cwd, readPath);
      // If direct path fails, try to find the file by searching for the filename
      if (!fsFile) {
        var fileName = path.basename(readPath);
        var suffix = readPath; // e.g. "training/minimal-viable-training.md"
        var suffixMatch = null;
        var nameMatch = null;
        // Walk the project to find a matching path suffix, or fallback to filename
        function findFile(dir, depth) {
          ...
          try {
            ...
            for (var ei = 0; ei < entries.length; ei++) {
              ...
              if (e.isDirectory()) {
                findFile(full, depth + 1);
              } else if (e.name === fileName) {
                var rel = path.relative(cwd, full);
                if (rel.endsWith(suffix)) {
                  suffixMatch = full;
                } else if (!nameMatch) {
                  nameMatch = full; // first filename match as fallback
                }
              }
            }
          } catch (err) {}
        ...
        if (found) {
          fsFile = safePath(cwd, path.relative(cwd, found));
          if (fsFile) readPath = path.relative(cwd, found);
        }
      }
      if (!fsFile) {
        var testPath = path.resolve(cwd, readPath);
        if (testPath.startsWith(cwd + path.sep) && !fs.existsSync(testPath)) {
          sendTo(ws, { type: "fs_read_result", path: msg.path, error: "File not found" });
        } else {
      ...
      try {
        ...
        // Directory: generate a markdown listing of contents
        if (stat.isDirectory()) {
          var entries = fs.readdirSync(fsFile, { withFileTypes: true });
          var lines = ["# " + path.basename(fsFile), ""];
          var dirs = [], files = [];
          for (var ei = 0; ei < entries.length; ei++) {
            if (entries[ei].name.startsWith(".")) continue;
            if (entries[ei].isDirectory()) dirs.push(entries[ei].name);
            else files.push(entries[ei].name);
          }
    ...
    if (msg.type === "fs_file_history") {
      ...
      // Collect git commits (async)
      execFile("git", ["log", "--format=%H|%at|%an|%s", "--follow", "--", histPath],
        { cwd: cwd, encoding: "utf8", timeout: 5000 },
        function (err, gitLog) {
          if (!err && gitLog) {
    ...
    // --- Git status (async — never block the event loop) ---
    if (msg.type === "git_status") {
      execFile("git", ["status", "--porcelain", "-uall"],
        { cwd: cwd, encoding: "utf8", timeout: 10000 },
        function (err, statusOut) {
          if (err) {
            sendTo(ws, { type: "git_status_result", staged: [], changed: [], untracked: [], error: err.message });
            return;
          }
          var staged = [];
          var changed = [];
    ...
    if (msg.type === "git_stage") {
      ...
      execFile("git", ["add", "--"].concat(paths), { cwd: cwd, timeout: 5000 }, function (e) {
        if (e) sendTo(ws, { type: "git_action_result", action: "stage", error: e.message });
        else sendTo(ws, { type: "git_action_result", action: "stage", success: true });
      });
      return;
    }
    ...
    if (msg.type === "git_unstage") {
      ...
      execFile("git", ["reset", "HEAD", "--"].concat(uPaths), { cwd: cwd, timeout: 5000 }, function (e) {
        if (e) sendTo(ws, { type: "git_action_result", action: "unstage", error: e.message });
        else sendTo(ws, { type: "git_action_result", action: "unstage", success: true });
      });
      return;
    }
    ...
    if (msg.type === "git_discard") {
      ...
      execFile("git", ["checkout", "--"].concat(dPaths), { cwd: cwd, timeout: 5000 }, function (e) {
        if (e) sendTo(ws, { type: "git_action_result", action: "discard", error: e.message });
        else sendTo(ws, { type: "git_action_result", action: "discard", success: true });
      });
      return;
    }
    ...
    if (msg.type === "git_diff_working") {
      ...
      execFile("git", wdArgs, { cwd: cwd, encoding: "utf8", timeout: 5000 }, function (e, wdDiff) {
        if (e) sendTo(ws, { type: "git_diff_working_result", path: wdPath, diff: "", error: e.message });
        else sendTo(ws, { type: "git_diff_working_result", path: wdPath, staged: !!msg.staged, diff: wdDiff || "" });
      });
      return;
    }
    ...
    if (msg.type === "fs_git_diff") {
      ...
      execFile("git", gitDiffArgs, { cwd: cwd, encoding: "utf8", timeout: 5000 }, function (e, diff) {
        if (e) sendTo(ws, { type: "fs_git_diff_result", hash: hash, hash2: hash2, path: diffPath, diff: "", error: e.message });
        else sendTo(ws, { type: "fs_git_diff_result", hash: hash, hash2: hash2, path: diffPath, diff: diff || "" });
      });
      return;
    }
    ...
    if (msg.type === "fs_file_at") {
      ...
      execFile("git", ["show", atHash + ":" + atRelPath], { cwd: cwd, encoding: "utf8", timeout: 5000 }, function (e, content) {
        if (e) sendTo(ws, { type: "fs_file_at_result", hash: atHash, path: atPath, content: "", error: e.message });
        else sendTo(ws, { type: "fs_file_at_result", hash: atHash, path: atPath, content: content });
      });
      return;
    }
    ...
    if (msg.documents && msg.documents.length > 0) {
      ...
      var MAX_DOC_BASE64_LENGTH = 67 * 1024 * 1024; // ~67MB base64 ≈ 50MB decoded
      ...
      for (var di = 0; di < msg.documents.length; di++) {
        ...
        // Check document size before decoding
    ...
    if (!session.isProcessing) {
      ...
      // Lazy warmup on first actual query (not on WS connect — that blocks event loop)
