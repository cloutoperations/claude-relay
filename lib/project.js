...
function createProjectContext(opts) {
  ...
  function sendToSessionViewers(sessionId, obj) {
    ...
    for (var [ws, state] of clients) {
      ...
      if (state.sessionId === sessionId) {
        ws.send(data);
      } else if (state.popups.has(sessionId)) {
    ...
    if (t === "permission_request" || t === "done" || t === "status" || t === "ask_user") {
      ...
      for (var [ws, state] of clients) {
        if (state.sessionId !== sessionId && !state.popups.has(sessionId) && ws.readyState === 1) {
          ws.send(ambient);
        }
      }
    }
  ...
  var FILE_INDEX_TTL = 60000; // 60 seconds
        ...
        while (stack.length > 0 && batch < 50) {
          ...
          for (var i = 0; i < items.length; i++) {
            ...
            if (child.isDirectory()) {
              ...
              stack.push({ dir: path.join(item.dir, child.name), rel: rel });
            } else {
  ...
  var sdk = createSDKBridge({
    ...
    onSessionRekey: function(oldId, newId) {
      // Update all clients that were viewing the old session ID
      for (var [ws, state] of clients) {
        if (state.sessionId === oldId) {
          state.sessionId = newId;
        }
        // Also update popup viewers
        if (state.popups.has(oldId)) {
          state.popups.delete(oldId);
          state.popups.add(newId);
        }
      }
    },
  ...
  function handleConnection(ws) {
    ...
    if (targetSession) {
      clients.get(ws).sessionId = targetSession.cliSessionId;
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "load_more_history") {
      ...
      var before = Math.min(msg.before, session.history.length);
    ...
    if (msg.type === "new_session") {
      var newSession = sm.createSession(ws, sendTo, msg.accountId || null);
      if (msg.projectPath) {
        newSession.projectPath = msg.projectPath;
        sm.saveSessionFile(newSession);
      }
      clients.get(ws).sessionId = newSession.cliSessionId;
      return;
    }
    ...
    if (msg.type === "resume_session") {
      ...
      clients.get(ws).sessionId = resumed.cliSessionId;
      return;
    }
    ...
    if (msg.type === "popup_open") {
      ...
      var POPUP_HISTORY_LIMIT = 2000;
      var fromIdx = 0;
      if (total > POPUP_HISTORY_LIMIT) {
        fromIdx = sm.findTurnBoundary(popupSession.history, Math.max(0, total - POPUP_HISTORY_LIMIT));
      }
    ...
    if (msg.type === "popup_message") {
      ...
      cstate.sessionId = msg.sessionId;
      try {
        handleMessage(ws, { type: "message", text: msg.text });
      } finally {
        cstate.sessionId = origSessionId;
        cstate.popups.add(msg.sessionId);
      }
      return;
    }
      ...
      (async function () {
        ...
        try {
          ...
          for (var [clientWs, state] of clients) {
            if (state.sessionId === session.cliSessionId) {
    ...
    console.log("[project] Message for session:", session.cliSessionId, "_tempId:", !!session._tempId, "isProcessing:", session.isProcessing, "hasQuery:", !!session.queryInstance);
    ...
    if (!session.isProcessing) {
      ...
      if (!session.queryInstance) {
        sdk.startQuery(session, fullText, msg.images, resolveAccount(session));
      } else {
  ...
  function handleHTTP(req, res, urlPath) {
    ...
    if (req.method === "GET" && urlPath === "/api/board") {
      ...
      sm.sessions.forEach(function (session) {
        ...
        var s = { id: session.cliSessionId, title: session.title || "", isProcessing: session.isProcessing || false, accountId: session.accountId || null, createdAt: session.createdAt, lastActivity: session.lastActivity };
      ...
      var areas = areaDirs.map(function (areaName) {
        ...
        try { fs.accessSync(path.join(areaDir, "02-operations")); hasOperations = true; } catch(e) {}
    ...
    if (req.method === "POST" && urlPath === "/api/board/tag-session") {
      parseJsonBody(req).then(function (body) {
        ...
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end('{"ok":true}');
      }).catch(function () { res.writeHead(400); res.end("Bad request"); });
      return true;
    }
    ...
    if (req.method === "GET" && urlPath === "/api/board/strategy") {
      ...
      try {
        ...
        if (sessionDirs.length > 0) {
          ...
          result.sessionDate = latestDir.match(/^(\d{4}-\d{2}-\d{2})/)[1];
          ...
          try {
            ...
            // Parse allocation table: | **Track** | **X%** | What |
            var allocLines = summary.match(/\|[^|]*\|\s*\*\*\d+%\*\*[^|]*\|[^|]*\|/g);
            if (allocLines) {
            ...
            // Parse gap analysis table: | Area | Present | Desired | Gap | Urgency |
            var gapSection = summary.match(/## Step 3: Gap Analysis[\s\S]*?\n\n/);
            if (gapSection) {
              var gapLines = gapSection[0].match(/\|\s*[^|]+\s*\|\s*[^|]+\s*\|\s*[^|]+\s*\|\s*\*\*[^*]+\*\*\s*\|\s*\*\*([^*]+)\*\*\s*\|/g);
              if (gapLines) {
                for (var gi = 0; gi < gapLines.length; gi++) {
                  var gapMatch = gapLines[gi].match(/\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*\*\*([^*]+)\*\*\s*\|\s*\*\*([^*]+)\*\*\s*\|/);
                  if (gapMatch) {
                    result.gaps.push({ area: gapMatch[1].trim(), present: gapMatch[2].trim(), desired: gapMatch[3].trim(), gap: gapMatch[4].trim(), urgency: gapMatch[5].trim() });
                  }
                }
              }
            }
