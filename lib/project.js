...
function createProjectContext(opts) {
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "session_settings") {
      var session = getClientSession(ws);
      if (session) {
        session.querySettings = {};
        if (msg.effort) session.querySettings.effort = msg.effort;
        console.log("[project] Session settings updated:", JSON.stringify(session.querySettings));
      }
      return;
    }
    ...
    if (!session.title) {
      session.title = (msg.text || msg.documents ? (msg.documents[0] || {}).name || "Document" : "Image").substring(0, 50);
      sm.saveSessionFile(session);
      sm.broadcastSessionList();
    }
