...
function createSessionManager(opts) {
  ...
  function searchOldVersions(q, seenIds) {
    ...
    try {
      ...
      for (var i = 0; i < entries.length; i++) {
        ...
        try { if (fs.statSync(oldDir).isDirectory()) oldDirs.push(oldDir); } catch (e) { console.warn("[session] statSync old_versions failed:", oldDir, e.message); }
      }
    } catch (e) { console.warn("[session] readdirSync failed for relay base dir:", e.message); return results; }

    for (var di = 0; di < oldDirs.length; di++) {
      var files;
      try { files = fs.readdirSync(oldDirs[di]); } catch (e) { console.warn("[session] readdirSync failed:", oldDirs[di], e.message); continue; }
      for (var fi = 0; fi < files.length; fi++) {
        ...
        try {
          ...
          var firstLine = buf.toString("utf8", 0, bytesRead).split("\n")[0];
          meta = JSON.parse(firstLine);
        } catch (e) { console.warn("[session] Failed to read archived session meta:", filePath, e.message); continue; } finally {
          if (fd !== null) try { fs.closeSync(fd); } catch {}
        }
        ...
        if (!titleMatch && !idMatch) {
          ...
          } catch (e) { console.warn("[session] Failed to read archived session for search:", filePath, e.message); }
        }
  ...
  function searchOtherProjects(q, seenIds) {
    var results = [];
    var otherDirs;
    try { otherDirs = getOtherProjectDirs(); } catch (e) { console.warn("[session] getOtherProjectDirs failed:", e.message); return results; }

    for (var pi = 0; pi < otherDirs.length; pi++) {
      var relayDir = path.join(otherDirs[pi], ".claude-relay");
      var sessionsDirs;
      try { sessionsDirs = fs.readdirSync(relayDir); } catch (e) { console.warn("[session] readdirSync failed:", relayDir, e.message); continue; }
      ...
      for (var si = 0; si < sessionsDirs.length; si++) {
        ...
        try { if (!fs.statSync(dir).isDirectory()) continue; } catch (e) { console.warn("[session] statSync failed:", dir, e.message); continue; }

        // Scan both the dir itself and its _old_versions/ subdir
        var scanDirs = [dir];
        var oldDir = path.join(dir, "_old_versions");
        try { if (fs.statSync(oldDir).isDirectory()) scanDirs.push(oldDir); } catch (e) { console.warn("[session] statSync old_versions failed:", oldDir, e.message); }

        for (var sdi = 0; sdi < scanDirs.length; sdi++) {
          var files;
          try { files = fs.readdirSync(scanDirs[sdi]); } catch (e) { console.warn("[session] readdirSync failed:", scanDirs[sdi], e.message); continue; }
          for (var fi = 0; fi < files.length; fi++) {
            if (!files[fi].endsWith(".jsonl")) continue;
            ...
            try {
              ...
              var firstLine = buf.toString("utf8", 0, bytesRead).split("\n")[0];
              meta = JSON.parse(firstLine);
            } catch (e) { console.warn("[session] Failed to read cross-project session meta:", filePath, e.message); continue; } finally {
              if (fd !== null) try { fs.closeSync(fd); } catch {}
            }
            if (!meta || meta.type !== "meta" || !meta.cliSessionId) continue;
            ...
            if (!titleMatch && !idMatch) {
              try {
                var raw = fs.readFileSync(filePath, "utf8");
                contentMatch = raw.toLowerCase().indexOf(q) !== -1;
              } catch (e) { console.warn("[session] Failed to read cross-project session for search:", filePath, e.message); }
            }
