    ...
    if (accounts.length > 1) {
      ...
      try {
        ...
        for (var ei = 0; ei < entries.length; ei++) {
          ...
          // Deduplicate by cliSessionId — keep the copy with MORE history
          for (var oi = 0; oi < otherSessions.length; oi++) {
            ...
            } else if (otherSessions[oi].history.length > allLoaded[seenCli[cid]].history.length) {
              // Replace with the copy that has more history
              allLoaded[seenCli[cid]] = otherSessions[oi];
            }
          }
        }
      } catch (e) {}
