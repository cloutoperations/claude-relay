...
var SCROLLBACK_CHUNK_MAX = 64 * 1024; // 64 KB max per individual chunk
...
function createTerminalManager(opts) {
  ...
  function create(cols, rows) {
    ...
    pty.onData(function (data) {
      // Truncate oversized chunks before buffering
      if (data.length > SCROLLBACK_CHUNK_MAX) {
        data = data.substring(data.length - SCROLLBACK_CHUNK_MAX);
      }
      // Buffer scrollback
    ...
    pty.onExit(function (e) {
      ...
      // Auto-remove exited terminals after 5 minutes
      setTimeout(function () {
        var s = terminals.get(id);
        if (s && s.exited) { s.scrollback = []; s.scrollbackSize = 0; terminals.delete(id); }
      }, 5 * 60 * 1000);
  ...
  function detachAll(ws) {
    for (var session of terminals.values()) {
      session.subscribers.delete(ws);
      // Grace period before killing PTY — allows browser reconnect
      if (session.subscribers.size === 0 && session.pty && !session.exited) {
        var sid = session.id;
        if (session._killTimer) clearTimeout(session._killTimer);
        session._killTimer = setTimeout(function () {
          var s = terminals.get(sid);
          if (!s || s.exited || s.subscribers.size > 0) return; // reconnected or already dead
          try { s.pty.kill("SIGTERM"); } catch (e) {}
          var ptyRef = s.pty;
          setTimeout(function () {
            if (ptyRef && !s.exited) {
              try { ptyRef.kill("SIGKILL"); } catch (e) {}
            }
          }, 5000);
  ...
  function write(id, data) {
    ...
    if (session && session.pty && !session.exited) {
      try { session.pty.write(data); } catch (e) {
        console.warn("[terminal] write failed for terminal", id, ":", e.message);
      }
    }
  }
