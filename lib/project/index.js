...
function createProjectContext(opts) {
  ...
  function handleConnection(ws) {
    ...
    // Send session list with full metadata (including boardCardId + overlays)
    sendTo(ws, sm.getSessionListMessage());

    var requestedSessionId = null;
    var noDefaultSession = false;
    try {
      var urlParams = new URLSearchParams(ws._relayQuery || "");
      var sParam = urlParams.get("s");
      if (sParam) requestedSessionId = sParam;
      // Copilot mode: don't auto-assign a default session, let the client request one
      if (urlParams.get("nodefault") === "1") noDefaultSession = true;
    ...
    } else if (!noDefaultSession) {
      targetSession = sm.getDefaultSession();
    }

    // Serial message processing — prevents race conditions when two messages
    // arrive quickly and both modify session state (permissions, blocks, etc.)
    // Initial history replay is chained in so messages queue behind it.
    var messageProcessing = Promise.resolve();
    if (targetSession) {
      setClientSession(ws, targetSession.cliSessionId);
      messageProcessing = sm.switchSessionForClient(ws, targetSession.cliSessionId, sendTo);
    }

    ws.on("message", function (raw) {
      var msg;
      try { msg = JSON.parse(raw.toString()); } catch (e) { return; }
      messageProcessing = messageProcessing.then(function() {
        return handleMessage(ws, msg);
      }).catch(function(err) {
        console.error("[project] Serial message handler error:", err.message || err);
      });
    });
