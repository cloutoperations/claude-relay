    ...
    if (req.method === "GET" && urlPath === "/api/board") {
      ...
      try {
        ...
        for (var adi = 0; adi < allDirs.length; adi++) {
          ...
          try { var ds = await fs.promises.stat(path.join(gtdPath, d)); if (ds.isDirectory()) areaDirs.push(d); } catch(e) {}
        }
      } catch(e) {}
      ...
      for (var aIdx = 0; aIdx < areaDirs.length; aIdx++) {
        ...
        try {
          ...
          for (var pi = 0; pi < entries.length; pi++) {
            ...
            var stat;
            try { stat = await fs.promises.stat(entryPath); } catch(e) { continue; }
            if (stat.isDirectory()) {
              var subProjects = [];
              try {
        ...
        var hasInbox = false;
        try { await fs.promises.access(path.join(areaDir, "inbox.md")); hasInbox = true; } catch(e) {}
        ...
        try {
          ...
          for (var oi = 0; oi < opEntries.length; oi++) {
            ...
            var opStat;
            try { opStat = await fs.promises.stat(opPath); } catch(e) { continue; }
            ...
            if (isDir) {
              try {
                ...
                try { await fs.promises.access(opDocPath); } catch(e) {
