...
module.exports = {
  ...
  async new_session(ctx, ws, msg) {
    var newSession = await ctx.sm.createSession(ws, ctx.sendTo, msg.accountId || null, null, msg._requestId);
    if (msg.projectPath) {
    ...
    if (msg.projectPath || msg.viewerName) {
      await ctx.sm.saveSessionFile(newSession);
    }
  ...
  async resume_session(ctx, ws, msg) {
    ...
    var resumed = await ctx.sm.resumeSession(msg.cliSessionId, ws, ctx.sendTo);
  ...
  async switch_session(ctx, ws, msg) {
    if (msg.id && ctx.sm.sessions.has(msg.id)) {
      ...
      await ctx.sm.switchSessionForClient(ws, msg.id, ctx.sendTo);
  ...
  async popup_open(ctx, ws, msg) {
    ...
    if (msg.skip_history) {
      ...
      // Batch sends to yield event loop every 50 items
      var BATCH = 50;
      for (var phi = fromIdx; phi < total; phi += BATCH) {
        var end = Math.min(phi + BATCH, total);
        for (var pj = phi; pj < end; pj++) {
          ctx.sendTo(ws, Object.assign({}, popupSession.history[pj], { _popupSessionId: msg.sessionId }));
        }
        if (end < total) {
  ...
  async delete_session(ctx, ws, msg) {
    if (msg.id && ctx.sm.sessions.has(msg.id)) {
      await ctx.sm.deleteSession(msg.id, ctx.clients, ctx.sendTo);
    }
  },
  ...
  async rename_session(ctx, ws, msg) {
    if (msg.id && ctx.sm.sessions.has(msg.id) && msg.title) {
      ...
      await ctx.sm.saveSessionFile(s);
      ctx.sm.broadcastSessionList();
    }
  },

  async search_sessions(ctx, ws, msg) {
    var results = await ctx.sm.searchSessions(msg.query || "");
  ...
  async fork_session(ctx, ws, msg) {
    ...
    var forkedSession = await ctx.sm.createSession(ws, ctx.sendTo, sourceSession.accountId || null, sourceSession.projectPath || null, requestId);
    forkedSession._forkFrom = sourceSessionId;
    forkedSession.title = (sourceSession.title || "Session") + " (fork)";
    await ctx.sm.saveSessionFile(forkedSession);
    ctx.sm.broadcastSessionList();
  ...
  async archive_session(ctx, ws, msg) {
    ...
    if (archiveId && ctx.sm.sessions.has(archiveId)) {
      ...
      await ctx.sm.saveSessionFile(archiveSession);
      ctx.sm.broadcastSessionList();
  ...
  async unarchive_session(ctx, ws, msg) {
    ...
    if (unarchiveId && ctx.sm.sessions.has(unarchiveId)) {
      ...
      await ctx.sm.saveSessionFile(unarchiveSession);
      ctx.sm.broadcastSessionList();
  ...
  async set_session_status(ctx, ws, msg) {
    ...
    if (statusId && ctx.sm.sessions.has(statusId) && ["open", "done", "waiting"].indexOf(newStatus) !== -1) {
      ...
      await ctx.sm.saveSessionFile(statusSession);
      ctx.sm.broadcastSessionList();
    }
  },

  async update_session_meta(ctx, ws, msg) {
    var sessionId = msg.id || msg.sessionId;
    if (!sessionId || !ctx.sm.sessions.has(sessionId)) { console.log("[update_session_meta] session not found:", sessionId); return; }
    var s = ctx.sm.sessions.get(sessionId);
    var changed = false;
    if ("boardCardId" in msg) {
      s.boardCardId = msg.boardCardId || null;
      changed = true;
    }
    if (changed) {
      // Save to meta overlay (separate from CLI's .jsonl file)
      ctx.sm.saveMetaOverlay(sessionId, { boardCardId: s.boardCardId });
      ctx.sm.broadcastSessionList();
      console.log("[update_session_meta] saved overlay + broadcast:", sessionId.substring(0, 12), "boardCardId:", s.boardCardId);
    }
  },
