...
function createEventProcessor(ctx) {
  ...
  function processSDKMessage(session, parsed) {
    ...
    if (parsed.session_id && isAuthoritativeMessage && session._tempId) {
      ...
      sm.rekeySession(oldId, parsed.session_id).catch(function(e) { console.error("[sdk] rekey file error:", e.message); });
      ...
      sm.rekeySession(oldId2, parsed.session_id).catch(function(e) { console.error("[sdk] rekey file error:", e.message); });
  ...
  function processStreamEvent(session, evt) {
    ...
    if (evt.type === "content_block_stop") {
      ...
      if (block && block.type === "tool_use") {
        var input = {};
        try { input = JSON.parse(block.inputJson); } catch (e) { console.warn("[sdk] Failed to parse tool input JSON for", block.name); }
  ...
  function handleExpiredSession(session, parsed, errMsg) {
    ...
    sm.saveSessionFile(session).catch(function(e) { console.error("[sdk] expire save error:", e.message); });
