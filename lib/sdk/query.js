...
function createQueryManager(ctx) {
  ...
  async function processQueryStream(session) {
    try {
      ...
      if (session.isProcessing) {
        ...
        } else if (err.message && err.message.indexOf("exited with code") !== -1 && (session._retryCount || 0) < 2) {
          session._retryCount = (session._retryCount || 0) + 1;
          session._retried = true;
          var retryOldId = session.cliSessionId;
          console.log("[sdk] Process exited, retry " + session._retryCount + "/2 (was: " + (retryOldId || "new") + ")");
  ...
  async function getOrCreateRewindQuery(session, account) {
    ...
    var rewindAC = new AbortController();
    rewindOpts.abortController = rewindAC;
    var tempQuery = sdk.query({ prompt: mq, options: rewindOpts });
    (async function() { try { for await (var msg of tempQuery) {} } catch(e) { if (e && e.name !== "AbortError") console.warn("[sdk] rewind query error:", e.message); } })();
    return {
      query: tempQuery,
      isTemp: true,
      cleanup: function() { try { rewindAC.abort(); } catch(e) {} try { mq.end(); } catch(e) {} },
    };
  }
  ...
  async function startQuery(session, text, images, account) {
    ...
    session._expiredRetried = false; // allow future expired-session retries
    ...
    var queryOptions = {
      ...
      systemPrompt: buildSystemPrompt(session, skills, ctx.instancePrompt),
    ...
    processQueryStream(session).catch(function(err) {
      console.error("[sdk] processQueryStream unexpected error:", err.message, err.stack);
      // If processQueryStream's own try/catch/finally failed, force cleanup
      session.isProcessing = false;
      session.queryInstance = null;
      session.messageQueue = null;
      session.abortController = null;
      ...
      session.pendingAskUser = {};
      try {
        sendAndRecord(session, { type: "error", text: "Unexpected error: " + err.message });
        sendAndRecord(session, { type: "done", code: 1 });
        sm.broadcastSessionList();
      } catch (e) {
        console.error("[sdk] Failed to notify clients of error:", e.message);
      }
    });
  }
