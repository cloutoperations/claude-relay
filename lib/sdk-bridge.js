...
var AGENT_SKILLS = {
  "strategy": [
    ...
    "- Track session hygiene: find untagged sessions, rename bad titles, flag stale/orphaned work",
...
// Build a compact conversation transcript from session history for context replay.
// Used when an SDK session expires but we still have local history.
function buildConversationContext(session) {
  if (!session.history || session.history.length === 0) return "";
  var lines = [];
  var totalLen = 0;
  var MAX_CONTEXT_CHARS = 80000; // ~20K tokens — enough for solid context without blowing limits
...
function buildSystemPrompt(session, skills, _instancePrompt) {
  var append = BASE_SYSTEM_APPEND;

  // Instance-level system prompt (e.g. copilot preamble — always injected)
  if (_instancePrompt) {
    append = append + "\n\n" + _instancePrompt;
  }
  ...
  // Expired session context replay: inject prior conversation so Claude doesn't lose the thread
...
function createSDKBridge(opts) {
    ...
    if (parsed.type === "stream_event" && parsed.event) {
      ...
      if (parsed.subtype === "error_max_turns") errorCategory = "max_turns";
        ...
        if (errMsg.indexOf("No conversation found") !== -1 || errMsg.indexOf("No message found") !== -1) {
          // Session or message expired on Anthropic's side — auto-retry with context replay
          ...
          // Save the expired session so history persists across daemon restarts
          sm.saveSessionFile(session);

          // Auto-retry: replay last user message with conversation context injected
          if (hasHistory && !session._expiredRetried) {
            ...
            if (lastText) {
              ...
              startQuery(session, lastText, null, session._account);
              return; // skip the normal done/result flow
            }
          }
  ...
  function handleCanUseTool(session, toolName, input, opts) {
    // Instance-level tool restrictions (e.g. copilot relay with limited access)
    if (toolRestrictions) {
      var trAllowed = toolRestrictions.allowedTools || [];
      if (trAllowed.indexOf(toolName) === -1) {
        return Promise.resolve({
          behavior: "deny",
          message: "Tool '" + toolName + "' is not available in this environment.",
        });
      }
      // Path restrictions for Read/Glob/Grep
      if ((toolName === "Read" || toolName === "Glob" || toolName === "Grep") && toolRestrictions.Read) {
        var trPaths = toolRestrictions.Read.allowedPaths || [];
        var trFilePath = (input && (input.file_path || input.path)) || "";
        if (trFilePath) {
          var trResolved = path.resolve(cwd, trFilePath);
          var trPathOk = false;
          for (var tri = 0; tri < trPaths.length; tri++) {
            if (trResolved.startsWith(path.resolve(cwd, trPaths[tri]))) { trPathOk = true; break; }
          }
          if (!trPathOk) {
            return Promise.resolve({
              behavior: "deny",
              message: "Access restricted to training materials only.",
            });
          }
        }
      }
      // Command restrictions for Bash
      if (toolName === "Bash" && toolRestrictions.Bash && input && input.command) {
        var trPattern = toolRestrictions.Bash.allowedPattern;
        if (trPattern && !new RegExp(trPattern).test(input.command)) {
          return Promise.resolve({
            behavior: "deny",
            message: "Only allowed commands can be run in this environment.",
          });
        }
        var trBlocked = toolRestrictions.Bash.blockedChars || [];
        for (var trj = 0; trj < trBlocked.length; trj++) {
          if (input.command.indexOf(trBlocked[trj]) !== -1) {
            return Promise.resolve({
              behavior: "deny",
      ...
      // Auto-approve tools that pass all restriction checks
  ...
  async function startQuery(session, text, images, account) {
    ...
    var queryOptions = {
      ...
      systemPrompt: buildSystemPrompt(session, skills, instancePrompt),
