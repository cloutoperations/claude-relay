  ...
  function processSDKMessage(session, parsed) {
    ...
    if (parsed.type === "stream_event" && parsed.event) {
      ...
      if (parsed.subtype === "error_max_turns") errorCategory = "max_turns";
        ...
        if (errMsg.indexOf("rate_limit") !== -1 || errMsg.indexOf("rate limit") !== -1) errorCategory = "rate_limit";
    ...
    try {
      ...
      if (session.isProcessing) {
        if (err.name === "AbortError" || (session.abortController && session.abortController.signal.aborted)) {
          ...
          session.cliSessionId = null; // don't resume a dead session
          ...
          startQuery(session, lastText || "continue", null);
          return;
        } else {
          session.isProcessing = false;
