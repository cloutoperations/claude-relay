    ...
    if (parsed.type === "system" && parsed.subtype === "init") {
      ...
      if (parsed.slash_commands) {
        ...
        sm.slashCommands = filteredNames.map(function(name) {
          ...
          } catch (e) { console.warn("[sdk] Failed to read slash command file:", name, e.message); }
    ...
    if (parsed.type === "stream_event" && parsed.event) {
      ...
      if (evt.type === "content_block_stop") {
        ...
        if (block && block.type === "tool_use") {
          var input = {};
          try { input = JSON.parse(block.inputJson); } catch (e) { console.warn("[sdk] Failed to parse tool input JSON for", block.name, ":", e.message); }
