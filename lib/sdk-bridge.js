...
function buildSystemPrompt(session) {
  ...
  var skillKey = path.split("/")[0];
  if (skillKey && AGENT_SKILLS[skillKey]) {
    append = append + "\n" + AGENT_SKILLS[skillKey];
    console.log("[sdk] Injecting agent skill for projectPath:", path, "→", skillKey);
  }
...
function createSDKBridge(opts) {
  ...
  function processSDKMessage(session, parsed) {
    ...
    if (parsed.session_id && session._tempId) {
    ...
    } else if (parsed.session_id && parsed.session_id !== session.cliSessionId) {
    ...
    if (parsed.type === "stream_event" && parsed.event) {
      ...
      if (parsed.subtype === "error_max_turns") errorCategory = "max_turns";
        ...
        else if (errMsg.indexOf("rate_limit") !== -1 || errMsg.indexOf("rate limit") !== -1) errorCategory = "rate_limit";
        else if (errMsg.indexOf("authentication") !== -1) errorCategory = "auth_failed";
