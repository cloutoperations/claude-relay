...
export function initInput(_ctx) {
  ...
  // Send/Stop button
  // When processing: if input has text, send it (continue talking); if empty, stop
  ctx.sendBtn.addEventListener("click", function () {
    if (ctx.processing && ctx.connected) {
      var hasInput = ctx.inputEl.value.trim().length > 0 || pendingImages.length > 0 || pendingPastes.length > 0;
      if (hasInput) {
        sendMessage();
      } else {
