...
var prevTodoStatuses = {};
var todoStartTimes = {};
var todoTimerInterval = null;
var todoCelebrationTimeout = null;
...
function isPlanFile(filePath) {
  return filePath && /\.claude(-\d+)?\/plans\//.test(filePath);
}
  ...
  for (var i = 0; i < todoItems.length; i++) {
    if (todoItems[i].id === input.taskId) {
      if (input.status === "deleted") {
        ...
        delete todoStartTimes[input.taskId];
      } else {
        if (input.status) {
          if (input.status === "in_progress" && todoItems[i].status !== "in_progress") {
            todoStartTimes[input.taskId] = Date.now();
          } else if (input.status !== "in_progress") {
            delete todoStartTimes[input.taskId];
          }
          todoItems[i].status = input.status;
        }
...
function todoStats() {
  ...
  for (var i = 0; i < todoItems.length; i++) {
    ...
    if (!inProgressItem && todoItems[i].status === "in_progress") inProgressItem = todoItems[i];
  }
  return {
    completed: completed,
    total: todoItems.length,
    pct: todoItems.length > 0 ? Math.round(completed / todoItems.length * 100) : 0,
    hasActive: hasActive,
...
function renderTodoItemsHtml(classPrefix) {
  ...
  for (var i = 0; i < sorted.length; i++) {
    ...
    var changed = prevTodoStatuses[t.id] !== undefined && prevTodoStatuses[t.id] !== t.status;
    var elapsedHtml = "";
    if (t.status === "in_progress" && todoStartTimes[t.id]) {
      elapsedHtml = ' <span class="' + classPrefix + 'elapsed" data-task-id="' + t.id + '">' + formatElapsed(Date.now() - todoStartTimes[t.id]) + '</span>';
    }
    html += '<div class="' + classPrefix + 'item ' + statusClass + (changed ? ' status-changed' : '') + '" data-task-id="' + t.id + '">' +
      '<span class="' + classPrefix + 'item-icon">' + todoStatusIcon(t.status) + '</span>' +
      '<span class="' + classPrefix + 'item-text">' + escapeHtml(t.status === "in_progress" && t.activeForm ? t.activeForm : t.content) + '</span>' +
      elapsedHtml +
      '</div>';
  }
  ...
  var newStatuses = {};
  for (var i = 0; i < todoItems.length; i++) {
    newStatuses[todoItems[i].id] = todoItems[i].status;
  }
  prevTodoStatuses = newStatuses;
  return html;
}

function startElapsedTimer() {
  if (todoTimerInterval) return;
  todoTimerInterval = setInterval(function () {
    var now = Date.now();
    // Update inline widget elapsed spans
    var spans = document.querySelectorAll(".todo-elapsed, .todo-sticky-elapsed");
    for (var i = 0; i < spans.length; i++) {
      var taskId = spans[i].getAttribute("data-task-id");
      if (taskId && todoStartTimes[taskId]) {
        spans[i].textContent = formatElapsed(now - todoStartTimes[taskId]);
      }
    }
  }, 1000);
}
...
function stopElapsedTimer() {
  if (todoTimerInterval) { clearInterval(todoTimerInterval); todoTimerInterval = null; }
}
...
function renderTodoWidget() {
  ...
  var html = '<div class="todo-header">' +
    ...
    '<span class="todo-header-count">' + stats.completed + '/' + stats.total + '</span>' +
    '</div>';
  html += '<div class="todo-progress"><div class="todo-progress-bar' + (stats.inProgressItem ? ' active' : '') + '" style="width:' + stats.pct + '%"></div></div>';
  html += '<div class="todo-items">';
  html += renderTodoItemsHtml("todo-");
  html += '</div>';
  ...
  // Start/stop elapsed timer based on whether any task is in_progress
  if (stats.inProgressItem) { startElapsedTimer(); } else { stopElapsedTimer(); }
...
function updateTodoSticky() {
  ...
  if (todoCelebrationTimeout) { clearTimeout(todoCelebrationTimeout); todoCelebrationTimeout = null; }
  stickyEl.classList.remove("todo-fade-out");

  // Compute dynamic top offset below header + file-viewer-tabs + banners
  var topOffset = 0;
  var headerEl = document.getElementById("header");
  if (headerEl) topOffset += headerEl.offsetHeight;
  var tabsEl = document.getElementById("file-viewer-tabs");
  if (tabsEl && tabsEl.offsetHeight > 0) topOffset += tabsEl.offsetHeight;
  var onboardingEl = document.getElementById("onboarding-banner");
  if (onboardingEl && !onboardingEl.classList.contains("hidden")) topOffset += onboardingEl.offsetHeight;
  var skipPermsEl = document.getElementById("skip-perms-banner");
  if (skipPermsEl && !skipPermsEl.classList.contains("hidden")) topOffset += skipPermsEl.offsetHeight;
  stickyEl.style.top = topOffset + "px";

  var stats = todoStats();

  // "All done!" celebration
  if (!stats.hasActive && stats.total > 0) {
    var wasCollapsed = stickyEl.classList.contains("collapsed");
    var html = '<div class="todo-sticky-inner todo-done-inner">' +
      '<div class="todo-sticky-header">' +
      '<span class="todo-sticky-icon">' + iconHtml("check-circle") + '</span>' +
      '<span class="todo-sticky-title">All done!</span>' +
      '<span class="todo-sticky-count">' + stats.completed + '/' + stats.total + '</span>' +
      '</div>' +
      '<div class="todo-sticky-progress"><div class="todo-sticky-progress-bar" style="width:100%"></div></div>' +
      '</div>';
    ...
    stickyEl.classList.remove("collapsed");
    refreshIcons();
    // Fade out after 2s
    todoCelebrationTimeout = setTimeout(function () {
      stickyEl.classList.add("todo-fade-out");
      setTimeout(function () {
        stickyEl.classList.add("hidden");
        stickyEl.classList.remove("todo-fade-out");
      }, 500);
  ...
  var html = '<div class="todo-sticky-inner">' +
    ...
    (stats.inProgressItem ? '<span class="todo-sticky-active">' + iconHtml("loader", "icon-spin") + ' ' + escapeHtml(stats.inProgressItem.activeForm || stats.inProgressItem.content) + '</span>' : '') +
    '<span class="todo-sticky-count">' + stats.completed + '/' + stats.total + '</span>' +
    '<button class="todo-sticky-scroll" title="Scroll to tasks">' + iconHtml("arrow-down-to-line") + '</button>' +
    '<span class="todo-sticky-chevron">' + iconHtml("chevron-down") + '</span>' +
    '</div>' +
    '<div class="todo-sticky-progress"><div class="todo-sticky-progress-bar' + (stats.inProgressItem ? ' active' : '') + '" style="width:' + stats.pct + '%"></div></div>' +
    '<div class="todo-sticky-items">';

  html += renderTodoItemsHtml("todo-sticky-");
  html += '</div></div>';
  stickyEl.innerHTML = html;
  ...
  // Scroll-to-widget button
  var scrollBtn = stickyEl.querySelector(".todo-sticky-scroll");
  if (scrollBtn) {
    scrollBtn.addEventListener("click", function (e) {
      e.stopPropagation();
      if (todoWidgetEl) {
        todoWidgetEl.scrollIntoView({ behavior: "smooth", block: "center" });
      }
    });
  }
