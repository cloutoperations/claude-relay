...
import { closeSidebar, switchTab } from './sidebar.js';
...
// --- Tab state ---
var openTabs = [];        // [{path, content, isMarkdown, isRendered, scrollTop}]
var activeTabPath = null;  // path of currently active tab

// --- File search state ---
var fileSearchActive = false;
var fileSearchResults = [];
var fileSearchActiveIndex = -1;
var fileSearchDebounce = null;

export function showFileSearch() {
  var modal = document.getElementById("file-search-modal");
  var input = document.getElementById("file-search-input");
  var results = document.getElementById("file-search-results");
  if (!modal) return;
  modal.classList.remove("hidden");
  input.value = "";
  results.innerHTML = "";
  fileSearchResults = [];
  fileSearchActiveIndex = -1;
  fileSearchActive = true;
  setTimeout(function () { input.focus(); }, 50);
}

export function hideFileSearch() {
  var modal = document.getElementById("file-search-modal");
...
export function handleFileSearchResult(msg) {
  if (!fileSearchActive) return;
  fileSearchResults = msg.results || [];
  fileSearchActiveIndex = fileSearchResults.length > 0 ? 0 : -1;
  renderFileSearchResults();
}

function renderFileSearchResults() {
  var container = document.getElementById("file-search-results");
  if (!container) return;
  ...
  for (var i = 0; i < fileSearchResults.length; i++) {
    var r = fileSearchResults[i];
    var iconClass = getFileIconClass(r.name);
    var dir = r.path.indexOf("/") !== -1 ? r.path.substring(0, r.path.lastIndexOf("/")) : "";
    html += '<div class="file-search-item' + (i === fileSearchActiveIndex ? ' active' : '') + '" data-index="' + i + '">' +
      '<span class="file-search-item-icon ' + iconClass + '"></span>' +
      '<span class="file-search-item-name">' + escapeHtml(r.name) + '</span>' +
      '<span class="file-search-item-path">' + escapeHtml(dir) + '</span>' +
      '</div>';
  }
  container.innerHTML = html;

  // Click handlers
  var items = container.querySelectorAll(".file-search-item");
  for (var j = 0; j < items.length; j++) {
    (function (idx) {
      items[idx].addEventListener("click", function () {
        selectFileSearchResult(idx);
      });
    })(j);
  }

  // Scroll active into view
  scrollFileSearchActive();
}

function scrollFileSearchActive() {
  var container = document.getElementById("file-search-results");
  if (!container || fileSearchActiveIndex < 0) return;
  var active = container.querySelector(".file-search-item.active");
  if (active) active.scrollIntoView({ block: "nearest" });
}

function selectFileSearchResult(index) {
  if (index < 0 || index >= fileSearchResults.length) return;
  var result = fileSearchResults[index];
  hideFileSearch();
...
function initFileSearch() {
  ...
  if (backdrop) {
    backdrop.addEventListener("click", function () { hideFileSearch(); });
  }
  ...
  input.addEventListener("input", function () {
    ...
    fileSearchDebounce = setTimeout(function () {
      if (ctx.ws && ctx.connected) {
        ctx.ws.send(JSON.stringify({ type: "fs_search", query: query }));
      }
    }, 150);
  });
  ...
  input.addEventListener("keydown", function (e) {
    ...
    if (e.key === "ArrowDown") {
      ...
      fileSearchActiveIndex = (fileSearchActiveIndex + 1) % fileSearchResults.length;
      renderFileSearchResults();
      return;
    }
    if (e.key === "ArrowUp") {
      e.preventDefault();
      if (fileSearchResults.length === 0) return;
      fileSearchActiveIndex = (fileSearchActiveIndex - 1 + fileSearchResults.length) % fileSearchResults.length;
      renderFileSearchResults();
      return;
    }
    if (e.key === "Enter") {
      e.preventDefault();
      if (fileSearchActiveIndex >= 0) {
        selectFileSearchResult(fileSearchActiveIndex);
      }
      return;
    }
  });
...
export function initFileBrowser(_ctx) {
  ...
  // Markdown render toggle — markdown always renders, so this is a no-op
  document.getElementById("file-viewer-render").addEventListener("click", function () {
    if (!currentContent || !currentIsMarkdown) return;
  });

  // PDF download button
  document.getElementById("file-viewer-pdf").addEventListener("click", generatePdf);

  // Header files toggle button
  var filesToggleBtn = document.getElementById("files-toggle-btn");
  if (filesToggleBtn) {
    filesToggleBtn.addEventListener("click", function () {
      if (openTabs.length === 0) return;
      if (!ctx.fileViewerEl.classList.contains("hidden")) {
        closeFileViewer();
      } else {
        activateTab(activeTabPath || openTabs[0].path);
      }
      updateFilesToggleBtn();
    });
  }
  ...
  // Search button
  var searchBtn = document.getElementById("file-panel-search");
  if (searchBtn) {
  ...
  // ESC to close, Cmd/Ctrl+W to close active tab
  document.addEventListener("keydown", function (e) {
    if (e.key === "Escape" && fileSearchActive) return; // handled by search input
    if (e.key === "Escape" && !ctx.fileViewerEl.classList.contains("hidden")) {
      closeFileViewer();
    }
    if (e.key === "w" && (e.metaKey || e.ctrlKey) && !e.shiftKey && activeTabPath && !ctx.fileViewerEl.classList.contains("hidden")) {
      e.preventDefault();
  ...
  tooltip.className = "file-tree-tooltip";
  document.body.appendChild(tooltip);
  var tooltipTimer = null;

  ctx.fileTreeEl.addEventListener("mouseover", function (e) {
    var item = e.target.closest(".file-tree-item");
    if (!item || !item.title) { clearTimeout(tooltipTimer); tooltip.classList.remove("visible"); return; }
  ...
  ctx.fileTreeEl.addEventListener("mouseout", function (e) {
    var item = e.target.closest(".file-tree-item");
    if (item) { clearTimeout(tooltipTimer); tooltip.classList.remove("visible"); }
  });

  // --- Divider drag-to-resize ---
  var divider = document.getElementById("file-viewer-divider");
  if (divider) {
    var dragging = false;
    ...
    divider.addEventListener("mousedown", function (e) {
      ...
      ctx.fileViewerEl.classList.add("dragging");
    ...
    document.addEventListener("mousemove", function (e) {
      ...
      var newWidth = layoutRect.right - e.clientX;
      var maxWidth = layoutRect.width * 0.8;
      if (newWidth < 280) newWidth = 280;
      if (newWidth > maxWidth) newWidth = maxWidth;
      ctx.fileViewerEl.style.width = newWidth + "px";
    });
    ...
    document.addEventListener("mouseup", function () {
      ...
      ctx.fileViewerEl.classList.remove("dragging");
    ...
    document.addEventListener("touchmove", function (e) {
      ...
      var layoutEl = document.getElementById("layout");
      var layoutRect = layoutEl.getBoundingClientRect();
      var newWidth = layoutRect.right - touch.clientX;
      var maxWidth = layoutRect.width * 0.8;
      if (newWidth < 280) newWidth = 280;
      if (newWidth > maxWidth) newWidth = maxWidth;
      ctx.fileViewerEl.style.width = newWidth + "px";
...
function openFileTab(path, content, isMarkdown) {
  var existing = null;
  for (var i = 0; i < openTabs.length; i++) {
    if (openTabs[i].path === path) { existing = openTabs[i]; break; }
  }
  if (existing) {
    existing.content = content;
    existing.isMarkdown = isMarkdown;
  } else {
    openTabs.push({ path: path, content: content, isMarkdown: isMarkdown, isRendered: isMarkdown, scrollTop: 0 });
  }
  activeTabPath = path;
  renderTabBar();
  persistTabs();
...
function activateTab(path) {
  ...
  isRendered = tab.isMarkdown ? true : tab.isRendered;
  ...
  // Always hide render button — markdown is always rendered
  renderBtn.classList.add("hidden");

  // Render content (or fetch if not yet loaded)
  ...
  if (tab.content != null) {
    if (tab.isMarkdown) {
      ...
      renderCodeWithLineNumbers(bodyEl, tab.content, ext);
    }
    // Restore scroll position
    requestAnimationFrame(function () {
      if (bodyEl) bodyEl.scrollTop = tab.scrollTop || 0;
    });
  } else {
    // Content not loaded yet — show loading and fetch
    bodyEl.innerHTML = '<div class="file-tree-loading">Loading...</div>';
  }

  // Always ensure viewer is visible
  ...
  // Reset history state for this tab
  ...
  // Sync sidebar — expand tree to file and highlight it
  switchTab("files");
...
function closeTab(path) {
  var idx = -1;
  for (var i = 0; i < openTabs.length; i++) {
    if (openTabs[i].path === path) { idx = i; break; }
  }
  if (idx === -1) return;
  ...
  if (activeTabPath === path) {
    ...
    var newIdx = idx < openTabs.length ? idx : openTabs.length - 1;
...
function renderTabBar() {
  var tabBarWrap = document.getElementById("file-viewer-tabs");
  var tabBar = document.getElementById("file-viewer-tabs-scroll");
  var header = document.getElementById("file-viewer-header");
  if (!tabBar || !tabBarWrap) return;
  ...
  for (var i = 0; i < openTabs.length; i++) {
    ...
    html += '<div class="file-viewer-tab' + (isActive ? ' active' : '') + '" data-tab-path="' + escapeHtml(tab.path) + '" title="' + escapeHtml(tab.path) + '">' +
      '<span class="file-viewer-tab-name">' + escapeHtml(fileName) + '</span>' +
      '<button class="file-viewer-tab-close" data-tab-close="' + escapeHtml(tab.path) + '">' + iconHtml("x") + '</button>' +
      '</div>';
  }
  tabBar.innerHTML = html;
  refreshIcons();

  // Event listeners + drag-to-reorder
  var tabs = tabBar.querySelectorAll(".file-viewer-tab");
  for (var j = 0; j < tabs.length; j++) {
    (function (tabEl) {
      var tabPath = tabEl.dataset.tabPath;
      tabEl.setAttribute("draggable", "true");

      tabEl.addEventListener("click", function (e) {
        if (e.target.closest(".file-viewer-tab-close")) return;
        if (tabPath !== activeTabPath) {
          activateTab(tabPath);
        } else if (!ctx.fileViewerEl.classList.contains("hidden")) {
          // Active tab clicked while viewer open — close it
          closeFileViewer();
        } else {
          // Active tab clicked while viewer closed — reopen
          activateTab(tabPath);
        }
      });
      // Middle-click to close
      tabEl.addEventListener("mousedown", function (e) {
        if (e.button === 1) { e.preventDefault(); closeTab(tabPath); }
      });
      ...
      tabEl.addEventListener("touchmove", function (e) {
        if (Math.abs(e.touches[0].clientX - touchStartX) > 10) touchDragging = true;
      }, { passive: true });
      tabEl.addEventListener("touchend", function (e) {
        if (!touchDragging) return;
        var touch = e.changedTouches[0];
        var dropTarget = document.elementFromPoint(touch.clientX, touch.clientY);
        var dropTab = dropTarget ? dropTarget.closest(".file-viewer-tab") : null;
        if (!dropTab || dropTab === tabEl) return;
        var dropPath = dropTab.dataset.tabPath;
        if (!dropPath) return;
        var fromIdx = -1, toIdx = -1;
        for (var k = 0; k < openTabs.length; k++) {
          if (openTabs[k].path === tabPath) fromIdx = k;
          if (openTabs[k].path === dropPath) toIdx = k;
        }
        if (fromIdx === -1 || toIdx === -1) return;
        var moved = openTabs.splice(fromIdx, 1)[0];
        toIdx = -1;
        for (var m = 0; m < openTabs.length; m++) {
          if (openTabs[m].path === dropPath) { toIdx = m; break; }
        }
        var rect = dropTab.getBoundingClientRect();
        var insertIdx = touch.clientX < rect.left + rect.width / 2 ? toIdx : toIdx + 1;
        openTabs.splice(insertIdx, 0, moved);
        persistTabs();
        renderTabBar();
      });
      ...
      tabEl.addEventListener("dragstart", function (e) {
        ...
        tabBar.classList.add("tab-dragging");
      });

      tabEl.addEventListener("dragend", function () {
        tabEl.classList.remove("dragging");
        tabBar.classList.remove("tab-dragging");
        var allTabs = tabBar.querySelectorAll(".file-viewer-tab");
        for (var k = 0; k < allTabs.length; k++) {
          allTabs[k].classList.remove("drop-left", "drop-right");
        }
      });
      ...
      tabEl.addEventListener("dragleave", function () {
        tabEl.classList.remove("drop-left", "drop-right");
      });

      tabEl.addEventListener("drop", function (e) {
        e.preventDefault();
        var draggedPath = e.dataTransfer.getData("text/plain");
        if (!draggedPath || draggedPath === tabPath) return;
        var fromIdx = -1, toIdx = -1;
        for (var k = 0; k < openTabs.length; k++) {
          if (openTabs[k].path === draggedPath) fromIdx = k;
          if (openTabs[k].path === tabPath) toIdx = k;
        }
        if (fromIdx === -1 || toIdx === -1) return;
        var rect = tabEl.getBoundingClientRect();
        var midX = rect.left + rect.width / 2;
        ...
        toIdx = -1;
        for (var m = 0; m < openTabs.length; m++) {
          if (openTabs[m].path === tabPath) { toIdx = m; break; }
        }
      ...
      var closeBtn = tabEl.querySelector(".file-viewer-tab-close");
      if (closeBtn) {
  ...
  var activeEl = tabBar.querySelector(".file-viewer-tab.active");
...
function saveActiveTabState() {
  ...
  var tab = getTab(activeTabPath);
  if (!tab) return;
  tab.isRendered = isRendered;
  tab.content = currentContent;
  var bodyEl = document.getElementById("file-viewer-body");
  if (bodyEl) tab.scrollTop = bodyEl.scrollTop;
}

function getTab(path) {
  for (var i = 0; i < openTabs.length; i++) {
    if (openTabs[i].path === path) return openTabs[i];
  }
  return null;
}
...
function persistTabs() {
  try {
    sessionStorage.setItem("claude-relay-file-tabs", JSON.stringify(openTabs.map(function (t) { return t.path; })));
  } catch (e) { /* ignore */ }
}

var tabsRestored = false;

export function restorePersistedTabs() {
  // Only restore once per page load (info message can fire on reconnect)
  if (tabsRestored) return;
  tabsRestored = true;

  try {
    var saved = sessionStorage.getItem("claude-relay-file-tabs");
    if (!saved) return;
    var paths = JSON.parse(saved);
    if (!Array.isArray(paths) || paths.length === 0) return;

    // Reset any existing tabs first
    openTabs = [];
    activeTabPath = null;
    ...
    // Create placeholder tabs immediately so the tab bar shows on load
    for (var j = 0; j < uniquePaths.length; j++) {
      openTabs.push({ path: uniquePaths[j], content: null, isMarkdown: false, isRendered: false, scrollTop: 0 });
    }
    activeTabPath = uniquePaths[0];
    renderTabBar();
    ...
    for (var k = 0; k < uniquePaths.length; k++) {
      requestFileContent(uniquePaths[k]);
    }
  } catch (e) { console.warn("restorePersistedTabs error:", e); }
}

function updateFilesToggleBtn() {
  var btn = document.getElementById("files-toggle-btn");
  var count = document.getElementById("files-tab-count");
  if (!btn) return;
  if (openTabs.length > 0) {
    btn.classList.remove("hidden");
    count.textContent = openTabs.length;
...
function syncMobileViewerTop() {
  if (window.innerWidth >= 1024) {
    ctx.fileViewerEl.style.top = "";
    return;
  }
  var tabs = document.getElementById("file-viewer-tabs");
  if (tabs && tabs.offsetHeight > 0) {
    ctx.fileViewerEl.style.top = tabs.getBoundingClientRect().bottom + "px";
  } else {
    ctx.fileViewerEl.style.top = "";
  }
}

function syncTabsPosition() {
  var tabs = document.getElementById("file-viewer-tabs");
  if (!tabs) return;
  var isDesktop = window.innerWidth >= 1024;
  var viewerOpen = !ctx.fileViewerEl.classList.contains("hidden");

  if (isDesktop && viewerOpen) {
    if (tabs.parentNode !== ctx.fileViewerEl) {
      ctx.fileViewerEl.insertBefore(tabs, ctx.fileViewerEl.firstChild);
    }
  } else {
    var parent = document.getElementById("workspace-main") || document.getElementById("app");
    var anchor = document.getElementById("connect-overlay");
    if (parent && anchor && tabs.parentNode !== parent) {
      parent.insertBefore(tabs, anchor);
    }
  }
}
...
function closeFileViewerInternal() {
  sendUnwatch();
  inlineDiffActive = false;
  ctx.fileViewerEl.classList.remove("file-viewer-wide");
  ctx.fileViewerEl.classList.add("hidden");
  ctx.fileViewerEl.style.width = "";
  ctx.fileViewerEl.style.top = "";
  var divider = document.getElementById("file-viewer-divider");
...
export function closeFileViewer() {
  // Just hide the viewer — tabs persist so they reappear when a file is opened
  saveActiveTabState();
  closeFileViewerInternal();
  updateFilesToggleBtn();
}
...
// --- Reveal file in tree (expand parents, highlight, scroll) ---
var pendingRevealPath = null; // relative file path to reveal
var pendingRevealSegments = null; // remaining dir segments to expand

function toRelativePath(filePath) {
  if (!filePath) return filePath;
  // Already relative
  if (filePath.charAt(0) !== "/") return filePath;
  // Convert absolute path to relative using project cwd
  if (ctx.cwd) {
    var prefix = ctx.cwd.endsWith("/") ? ctx.cwd : ctx.cwd + "/";
    if (filePath.indexOf(prefix) === 0) {
      return filePath.substring(prefix.length);
    }
  }
  // Fallback: try to match against known root entries
  if (treeData["."] && treeData["."].children) {
    for (var i = 0; i < treeData["."].children.length; i++) {
      var name = treeData["."].children[i].name;
      var idx = filePath.indexOf("/" + name + "/");
      if (idx !== -1) {
...
function revealFileInTree(filePath) {
  if (!filePath) return;
  var relPath = toRelativePath(filePath);
  // If still absolute, we can't reveal it
  if (relPath.charAt(0) === "/") return;
  pendingRevealPath = relPath;
  // Build list of directory paths to expand: "." then each parent
  // e.g. "gtd/1-next/foo.md" -> [".", "gtd", "gtd/1-next"]
  var parts = relPath.split("/");
  var dirs = ["."];
  for (var i = 0; i < parts.length - 1; i++) {
    dirs.push(parts.slice(0, i + 1).join("/"));
  }
  pendingRevealSegments = dirs;
  revealNextSegment();
}

function revealNextSegment() {
  if (!pendingRevealSegments || pendingRevealSegments.length === 0) {
    // All dirs expanded, now highlight the file
    highlightAndScrollToFile(pendingRevealPath);
    pendingRevealSegments = null;
    return;
  }
  var dirPath = pendingRevealSegments.shift();
  if (treeData[dirPath] && treeData[dirPath].loaded) {
    // Already loaded — just expand the DOM node and continue
    expandDirInDom(dirPath);
    revealNextSegment();
  } else {
    // Need to load — request it; handleFsList will call revealNextSegment
    expandDirInDom(dirPath);
    requestDirectory(dirPath);
  }
}

function expandDirInDom(dirPath) {
  if (dirPath === ".") return; // root is always shown
  var childEl = ctx.fileTreeEl.querySelector('.file-tree-children[data-parent-path="' + dirPath + '"]');
  if (childEl) {
    childEl.classList.remove("hidden");
    var row = childEl.previousElementSibling;
    if (row) row.classList.add("expanded");
  }
}

function highlightAndScrollToFile(filePath) {
  if (!filePath) return;
  var prev = ctx.fileTreeEl.querySelector(".file-tree-item.active");
  if (prev) prev.classList.remove("active");
  var fileName = filePath.split("/").pop();
  var parentDir = filePath.indexOf("/") !== -1 ? filePath.substring(0, filePath.lastIndexOf("/")) : ".";
  // Find the correct container and match by name within it
  var selector = parentDir === "." ? "#file-tree" : '.file-tree-children[data-parent-path="' + parentDir + '"]';
  var container = ctx.fileTreeEl.querySelector(selector) || ctx.fileTreeEl;
  var items = container.querySelectorAll(":scope > .file-tree-item");
  for (var i = 0; i < items.length; i++) {
    var nameEl = items[i].querySelector(".file-tree-name");
  ...
  // Fallback: search entire tree if not found in expected container
  var allItems = ctx.fileTreeEl.querySelectorAll(".file-tree-item");
  for (var j = 0; j < allItems.length; j++) {
    var nameEl2 = allItems[j].querySelector(".file-tree-name");
    if (nameEl2 && nameEl2.textContent === fileName) {
      allItems[j].classList.add("active");
      allItems[j].scrollIntoView({ block: "center", behavior: "smooth" });
      return;
    }
  }
}
...
export function openFile(filePath, opts) {
  ...
  // Switch sidebar to Files tab and reveal file in tree
  switchTab("files");
...
function generatePdf() {
  ...
  if (ext !== "md" && ext !== "mdx") {
    alert("PDF generation is only supported for markdown files.");
    return;
  }
  ...
  fetch("api/pdf?path=" + encodeURIComponent(currentFilePath))
    .then(function (res) {
    ...
    .then(function (blob) {
      ...
      a.href = url;
      // Extract filename from content-disposition or build from path
      var baseName = currentFilePath.split("/").pop().replace(/\.[^.]+$/, "");
      a.download = baseName + " - Clout Operations.pdf";
...
export function refreshIfOpen(filePath) {
  ...
  // Check all open tabs — refresh any that match
  for (var i = 0; i < openTabs.length; i++) {
    var tabPath = openTabs[i].path;
    if (filePath === tabPath || filePath.endsWith("/" + tabPath)) {
      // For the active tab, do the full refresh with re-render
      if (tabPath === activeTabPath) {
        if (historyVisible || inlineDiffActive) continue;
        pendingRefresh = true;
        requestFileContent(tabPath);
      } else {
        // Background tabs: just fetch fresh content (handled by showFileContent cache update)
        requestFileContent(tabPath);
      }
    }
  }
...
export function handleFsList(msg) {
  ...
  if (msg.error) {
    ...
    // Don't break reveal chain on error — abort gracefully
    if (pendingRevealSegments) {
      pendingRevealSegments = null;
  ...
  if (dirPath === ".") {
    ...
    // Continue reveal if pending
    if (pendingRevealSegments) revealNextSegment();
    return;
  }
    ...
    if (entry.type === "dir") {
      ...
      (function (filePath, rowEl) {
        rowEl.addEventListener("click", function (e) {
          ...
          // If already open as a tab, just activate it
          if (getTab(filePath) && activeTabPath !== filePath) {
            activateTab(filePath);
          } else if (getTab(filePath) && activeTabPath === filePath) {
            // Already active — just ensure viewer is visible
            ctx.fileViewerEl.classList.remove("hidden");
            var dvdr = document.getElementById("file-viewer-divider");
            if (dvdr) dvdr.classList.remove("hidden");
            syncMobileViewerTop();
          } else {
            requestFileContent(filePath);
          }
...
function showFileContent(msg) {
  ...
  // If this is a background refresh for a non-active tab, just update the cache
  var isBackgroundUpdate = msg.path !== activeTabPath && getTab(msg.path) && !msg.error && !msg.binary;
  if (isBackgroundUpdate) {
    var cachedTab = getTab(msg.path);
    cachedTab.content = msg.content;
    return;
  }

  // If active tab already shows same content AND viewer is already visible, skip re-render
  var existingTab = getTab(msg.path);
  var viewerVisible = !ctx.fileViewerEl.classList.contains("hidden");
  if (viewerVisible && msg.path === activeTabPath && existingTab && !keepRenderState && !pendingOpenMode && !msg.error && !msg.binary) {
    if (existingTab.content === msg.content) {
      existingTab.content = msg.content;
      return;
    }
  }
  ...
  if (msg.error) {
    ...
    msgIsMarkdown = currentIsMarkdown;

    if (currentIsMarkdown) {
      renderBtn.classList.add("hidden");
    }

    // If keepRenderState, preserve the previous render toggle
    if (keepRenderState) {
      // isRendered was already preserved (not reset above)
    } else if (currentIsMarkdown) {
      isRendered = true;
    }

    if (currentIsMarkdown && isRendered) {
      renderBody();
    } else {
  ...
  // Register/update the tab (skip for errors and binaries — still open them but with null content)
  if (!msg.error) {
    openFileTab(msg.path, msg.binary ? null : msg.content, msgIsMarkdown);
  } else {
    openFileTab(msg.path, null, false);
  }

  // Reveal the active file in the tree (expand parents, highlight, scroll)
  if (msg.path === activeTabPath) {
    revealFileInTree(msg.path);
  }
...
export function handleFileChanged(msg) {
  ...
  // Update tab cache for any open tab with this path
  var tab = getTab(msg.path);
  if (tab && msg.content != null) {
    tab.content = msg.content;
  }

  // Only re-render if it's the active tab being viewed
