...
export function initFileBrowser(_ctx) {
  ...
  if (divider) {
    ...
    document.addEventListener("mousemove", function (e) {
      ...
      // Clamp between 280px and 80% of layout width
...
function activateTab(path) {
  ...
  if (tab.content != null) {
    if (tab.isMarkdown && tab.isRendered) {
      renderBody();
    } else {
...
function renderTabBar() {
  ...
  if (openTabs.length === 0) {
    ...
    if (header) header.classList.remove("tabs-active");
    return;
  }

  tabBarWrap.style.display = "";
  if (header) header.classList.add("tabs-active");
