  ...
  for (var j = 0; j < tabs.length; j++) {
    (function (tabEl) {
      ...
      tabEl.addEventListener("click", function (e) {
        ...
        if (tabPath !== activeTabPath) activateTab(tabPath);
      });
...
function persistTabs() {
  try {
    localStorage.setItem("claude-relay-file-tabs", JSON.stringify(openTabs.map(function (t) { return t.path; })));
  } catch (e) { /* ignore */ }
}
...
export function restorePersistedTabs() {
  ...
  try {
    var saved = localStorage.getItem("claude-relay-file-tabs");
    if (!saved) return;
...
function showFileContent(msg) {
  ...
  // Fallback: if no tree item is highlighted, try to reveal the file
  var activeItem = ctx.fileTreeEl.querySelector(".file-tree-item.active");
  if (!activeItem) {
    var relPath = toRelativePath(msg.path);
    highlightAndScrollToFile(relPath);
  }
