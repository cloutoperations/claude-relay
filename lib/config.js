...
function writeCrashInfo(info) {
  ...
  } catch (e) { console.warn("[config] Failed to write crash info:", e.message); }
}
...
/**
 * Detect all Claude accounts on this machine.
 * Scans ~/.claude, ~/.claude-1, ~/.claude-2, ~/.claude-foo, etc.
 * Returns [{ configDir, name, email }]
 */
function detectClaudeAccounts() {
  ...
  } catch (e) { console.warn("[config] Failed to scan home dir for Claude accounts:", e.message); }

  var accounts = [];
  for (var dj = 0; dj < dirs.length; dj++) {
    // Check both <configDir>/.claude.json and (for default) ~/.claude.json at root
    var candidates = [path.join(dirs[dj].path, ".claude.json")];
    if (dirs[dj].name === "default") {
      candidates.push(path.join(home, ".claude.json"));
    }
    for (var ck = 0; ck < candidates.length; ck++) {
      if (!fs.existsSync(candidates[ck])) continue;
      try {
        var auth = JSON.parse(fs.readFileSync(candidates[ck], "utf8"));
        if (auth.oauthAccount) {
          ...
          break; // found auth for this dir, stop checking candidates
        }
      } catch (e) { console.warn("[config] Failed to read Claude account auth:", candidates[ck], e.message); }
    }
  }
  return accounts;
}
...
module.exports = {
  ...
  detectClaudeAccounts: detectClaudeAccounts,
};
