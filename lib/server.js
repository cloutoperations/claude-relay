...
function createServer(opts) {
  ...
  server.on("upgrade", function (req, socket, head) {
    ...
    if (origin) {
      try {
        ...
        // Allow localhost dev servers (e.g. Vite on :5173) to connect
        var isLocalDev = originUrl.hostname === "localhost" || originUrl.hostname === "127.0.0.1";
        if (originPort !== String(portNum) && originPort !== hostPort && !isLocalDev) {
  ...
  // --- Local HTTP dev server (plain HTTP, shares all routes + WS with main server) ---
  var devServer = null;
  var devWss = null;
