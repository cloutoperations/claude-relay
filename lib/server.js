...
var { createProjectContext } = require("./project/index");

var { CONFIG_DIR } = require("./config");

var svelteDir = path.join(__dirname, "..", "svelte-frontend", "dist");
var publicDir = fs.existsSync(svelteDir) ? svelteDir : path.join(__dirname, "public");
var bundledThemesDir = path.join(__dirname, "themes");
var userThemesDir = path.join(CONFIG_DIR, "themes");

var MAX_BODY_SIZE = 100 * 1024 * 1024; // 100MB
...
var staticCache = new Map(); // filePath -> { content, mtime }
...
function serveStatic(urlPath, res) {
  ...
  try {
    ...
    var cacheControl = (ext === ".js" || ext === ".css" || ext === ".html") ? "no-store" : "no-cache";

    // Serve from in-memory cache if file hasn't changed
    var stat = fs.statSync(filePath);
    var cached = staticCache.get(filePath);
    ...
    staticCache.set(filePath, { content: content, mtime: stat.mtimeMs });
    res.writeHead(200, { "Content-Type": mime + "; charset=utf-8", "Cache-Control": cacheControl });
    res.end(content);
    return true;
  } catch (e) {
...
function createServer(opts) {
  ...
  var toolRestrictions = opts.toolRestrictions || null;
  var instancePrompt = opts.instancePrompt || null;
  var lanHost = opts.lanHost || null;
  var accountLabel = opts.accountLabel || null;
  var accounts = opts.accounts || [];
  var boardApiUrl = opts.boardApiUrl || null;
  ...
  try {
    ...
    pushModule = initPush();
  } catch (e) { console.warn("[server] Push module init failed:", e.message); }
  ...
  var appHandler = function (req, res) {
      ...
      req.on("end", function () {
        try {
          ...
          if (authToken && generateAuthToken(data.pin) === authToken) {
            clearPinFailures(ip);
            res.writeHead(200, {
              "Set-Cookie": "relay_auth=" + authToken + "; Path=/; HttpOnly; SameSite=Lax; Max-Age=31536000" + (tlsOptions ? "; Secure" : ""),
    ...
    if (req.method === "GET" && fullUrl === "/api/themes") {
      ...
      try {
        ...
        for (var i = 0; i < bFiles.length; i++) {
          ...
          } catch (e) { console.warn("[server] Failed to read bundled theme:", bFiles[i], e.message); }
        }
      } catch (e) { console.warn("[server] Failed to read bundled themes dir:", e.message); }
      try {
        ...
        for (var j = 0; j < uFiles.length; j++) {
          ...
          } catch (e) { console.warn("[server] Failed to read user theme:", uFiles[j], e.message); }
        }
      } catch (e) { console.warn("[server] Failed to read user themes dir:", e.message); }
      res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (fullUrl === "/" && req.method === "GET") {
      ...
      // Default project redirect — skip the picker
      if (!hasGoneParam && projects.has("clout-operations")) {
        res.writeHead(302, { "Location": "/p/clout-operations/" });
        res.end();
        return;
      }
      var statusList = [];
    ...
    // Try project HTTP handler first (APIs) — handleHTTP is async
    var origUrl = req.url;
    req.url = projectUrlWithQS;
    var handled = ctx.handleHTTP(req, res, projectUrlWithQS);
    req.url = origUrl;
    if (handled && typeof handled.then === "function") {
      handled.then(function (wasHandled) {
        if (!wasHandled) {
          if (req.method === "GET" && serveStatic(projectUrl, res)) return;
          res.writeHead(404);
          res.end("Not found");
        }
      }).catch(function (err) {
        console.error("[server] HTTP handler error:", err.message || err);
  ...
  // --- WebSocket ---
  var wss = new WebSocketServer({ noServer: true, maxPayload: 50 * 1024 * 1024 }); // 50MB for image attachments
  ...
  server.on("upgrade", function (req, socket, head) {
    ...
    if (!isAuthed(req, authToken)) {
      console.warn("[server] WS auth failed from:", req.socket.remoteAddress || "unknown");
  ...
  function addProject(cwd, slug, title) {
    ...
    var ctx = createProjectContext({
      ...
      boardApiUrl: boardApiUrl,
    });
    projects.set(slug, ctx);
    // Don't warmup on addProject — it spawns CLI processes that block the event loop.
    // Warmup happens on first WS connection instead (see handleConnection in project.js).
    return true;
  }
  ...
  // --- Local HTTP dev server (plain HTTP, skips auth, shares routes + WS) ---
  var devServer = null;
  var devWss = null;
  function startDevServer(devPort) {
    devServer = http.createServer(appHandler);
    devWss = new WebSocketServer({ noServer: true, maxPayload: 50 * 1024 * 1024 });

    devServer.on("upgrade", function (req, socket, head) {
      // Dev server skips auth — local dev only
      var wsSlug = extractSlug(req.url);
      if (!wsSlug) { socket.destroy(); return; }
      var ctx = projects.get(wsSlug);
      if (!ctx) { socket.destroy(); return; }
      devWss.handleUpgrade(req, socket, head, function (ws) {
        if (ws._socket) ws._socket._ws = true;
        var qIdx = req.url.indexOf("?");
        ws._relayQuery = qIdx >= 0 ? req.url.substring(qIdx + 1) : "";
        ctx.handleConnection(ws);
      });
    ...
    devServer.listen(devPort, function () {
      console.log("[daemon] Dev HTTP on http://localhost:" + devPort);
    });
    devServer.on("error", function (e) {
      console.warn("[daemon] Dev HTTP server error:", e.message);
      devServer = null;
    });
  }
